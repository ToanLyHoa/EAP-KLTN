...
def get_data(cfg):
    ...
    val = VideoIter(csv_filepath=os.path.join(labels_dir, 'val.csv'),
                    sampler=val_sampler,
    ...
    if eval_only:
        ...
        train = VideoIter(csv_filepath = os.path.join(labels_dir, 'train.csv'),
...
def create(cfg, return_train=True, return_len=False):
    if cfg.EVAL_ONLY:
        val = get_data(cfg, eval_only=True)
